		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
	            	...
	            	html += '						<div class="filePicker">点击上传图片</div>';
			...
			this.funFilterEligibleFile = function(files){
				...
				for (var i = 0, file; file = files[i]; i++) {
					if (file.size >= 51200000) {
						...
						jAlert('您这个"'+ file.name +'"文件大小过大', '消息提醒', function() {
						...
						//不是图片文件
						$.alerts.dialogClass = "style_1"; // 设置自定义样式的Class
						jAlert('文件'+ file.name+'不是图片文件！', '消息提醒', function() {
							$.alerts.dialogClass = null; // 重置到默认值
						});
						//alert('您这个"'+ file.name +'"文件大小过大');
				...
				var params = {
					...
					onSuccess: function(file, response) {
						if (response.indexOf("1")!=-1) {
							...
							//$("#uploadInf").append("<p>文件" + file.name + "上传失败,请重新上传！</p>");	
				...
				*/
				...
				$(".upload_btn").bind("click", function(e){
					...
					if(ZYFILE.funReturnNeedFiles().length > 0){
						...
						$.alerts.dialogClass = "style_1"; // 设置自定义样式的Class
						jAlert("请先选中文件再点击上传!", '消息提醒', function() {
							$.alerts.dialogClass = null; // 重置到默认值
						});
						//alert("请先选中文件再点击上传");
					}*/
