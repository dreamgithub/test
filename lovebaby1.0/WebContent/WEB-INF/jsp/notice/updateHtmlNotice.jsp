...
<body>
		...
		<div >
				<form action="${path }/webnotice/addnotice.jhtml" >
					<div style=" width: 100%;height: 50px;margin-top: 30px;margin-bottom: -20px;">
						<span style="font-weight:bold;margin-right: 20px ">标题</span><input type="text" name="title" style="width: 200px" id="title" value=" ${result.result.title }">
	...
	<div style="display:none;position: absolute;margin-top: -200px;" id="sel_district">
					...
					if (title=="") {
						...
						if (from_type=='1'||from_type=='4') {
							if ($("input[choose='all']:checkbox:checked").length==0) {								
								...
								$.post("${path }/webnotice/updateHtmlNotice.jhtml",{id:id,from_id:from_id,from_type:from_type,title:title,content:content,contentType:'1',to_type:to_type,toids:toids,uid:uid,textContent:textContent,picname:firstpicname},
										   function(data){
												if (data.flag=='1') {
													$.alerts.dialogClass = "style_1"; // 设置自定义样式的Class
								...
								var to_type="0";
								$.post("${path }/webnotice/updateHtmlNotice.jhtml",{id:id,from_id:from_id,from_type:from_type,title:title,content:content,contentType:'1',to_type:to_type,uid:uid,textContent:textContent,picname:firstpicname },
										   function(data){
									if (data.flag=='1') {
										$.alerts.dialogClass = "style_1"; // 设置自定义样式的Class
										jAlert("修改成功!", '消息提醒', function() {
