	...
	<div class="main-container">
		<nav class="main-nav">
			...
			<ul class="nav right center-text">
				<li id="unread" class="active">未审核申请</li>
				<li id="read" class="active btn">已审核申请</li>			
			</ul>
		</nav>
		<!-- 推荐主题 -->
		<div>
							...
							<tbody>
								<c:forEach items="${result.result }"  var="data"   varStatus="status">				
									<tr>
										...
										<td>${ data.area}</td>
										<td>
											<c:if test="${ data.status=='1'}">认证通过</c:if>
											<c:if test="${ data.status=='2'}">认证拒绝</c:if>
										</td>
										<td>${ data.date}</td>
									</tr>
								</c:forEach>								
